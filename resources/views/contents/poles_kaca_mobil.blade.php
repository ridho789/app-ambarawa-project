...
<div class="container">
    <div class="page-inner">
        ...
        <!-- Modal Export data -->
        <div class="modal fade" id="polesExportModal" tabindex="-1" role="dialog" aria-labelledby="polesExportModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header border-0 mx-2" style="margin-bottom: -25px;">
                        <h5 class="modal-title" id="polesExportModal">
                    ...
                    <form action="{{ url('poles-export') }}" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
                        @csrf
                        <div class="modal-body">
                            <p class="small mx-2">
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex align-items-center">
                            ...
                            <form id="statusPendingForm" method="POST" action="{{ url('poles-status_pending') }}" class="d-inline">
                                @csrf
                            ...
                            <form id="statusProcessForm" method="POST" action="{{ url('poles-status_process') }}" class="d-inline">
                                @csrf
                            ...
                            <form id="statusPaidForm" method="POST" action="{{ url('poles-status_paid') }}" class="d-inline">
                                @csrf
                            ...
                            <div class="ms-auto d-flex align-items-center">
                                @if (count($poles) > 0)
                                    <button class="btn btn-success btn-round ms-2 btn-sm" data-bs-toggle="modal" data-bs-target="#polesExportModal">
                                        <i class="fa fa-file-excel"></i>
                                        Export data
                                    </button>
                                @endif
                                <button class="btn btn-primary btn-round ms-3 btn-sm" data-bs-toggle="modal" data-bs-target="#polesModal">
                            ...
                            <table id="basic-datatables" class="display table table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr data-id="{{ $p->id_tagihan_amb }}" 
                                        ...
                                        @if ($p->file)
                                            <td>
                                                <a href="{{ asset('storage/' . $p->file) }}" target="_blank">{{ $p->pemesan ?? '-' }}</a>
                                            </td>
                                        @else
                                            <td>{{ $p->pemesan ?? '-' }}</td>
                                        @endif
