...
<div class="container">
    <div class="page-inner">
        <div class="page-header">
            <h3 class="fw-bold mb-3">Data Pengeluaran AC</h3>
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header border-0 mx-2" style="margin-bottom: -25px;">
                        <h5 class="modal-title" id="acModal">
                            <span class="fw-light"> Data</span>
                            <span class="fw-mediumbold"> Pengeluaran AC Baru </span>
                        </h5>
                        <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form action="{{ url('ac-store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
        ...
        <!-- Modal Edit Data -->
        <div class="modal fade" id="acEditModal" tabindex="-1" role="dialog" aria-labelledby="acEditModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header border-0 mx-2" style="margin-bottom: -25px;">
                        <h5 class="modal-title" id="acEditModal">
                            <span class="fw-light"> Data</span>
                            <span class="fw-mediumbold"> Pengeluaran AC </span>
                        </h5>
                        <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form action="{{ url('ac-update') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="modal-body">
                            <p class="small mx-2">
        ...
        <!-- Modal Export data -->
        <div class="modal fade" id="acExportModal" tabindex="-1" role="dialog" aria-labelledby="acExportModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header border-0 mx-2" style="margin-bottom: -25px;">
                        <h5 class="modal-title" id="acExportModal">
                    ...
                    <form action="{{ url('ac-export') }}" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
                        @csrf
                        <div class="modal-body">
                            <p class="small mx-2">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex align-items-center">
                            <div class="card-title">List Pengeluaran AC</div>
                            <input type="hidden" id="allSelectRow" name="ids" value="">
                            <button id="editButton" class="btn btn-warning btn-round ms-5 btn-sm" data-bs-toggle="modal" data-bs-target="#acEditModal" style="display: none;">
                                <i class="fa fa-edit"></i>
                                 Edit data
                            </button>
                            <form id="deleteForm" method="POST" action="{{ url('ac-delete') }}" class="d-inline">
                                @csrf
                            ...
                            <form id="statusPendingForm" method="POST" action="{{ url('ac-status_pending') }}" class="d-inline">
                                @csrf
                            ...
                            <form id="statusProcessForm" method="POST" action="{{ url('ac-status_process') }}" class="d-inline">
                                @csrf
                            ...
                            <form id="statusPaidForm" method="POST" action="{{ url('ac-status_paid') }}" class="d-inline">
                                @csrf
                            ...
                            <div class="ms-auto d-flex align-items-center">
                                @if (count($ac) > 0)
                                    <button class="btn btn-success btn-round ms-2 btn-sm" data-bs-toggle="modal" data-bs-target="#acExportModal">
                                        <i class="fa fa-file-excel"></i>
                                        Export data
                                    </button>
                                @endif
                                <button class="btn btn-primary btn-round ms-3 btn-sm" data-bs-toggle="modal" data-bs-target="#acModal">
                            ...
                            <table id="basic-datatables" class="display table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <!-- <th class="text-xxs-bold">Dipakai untuk</th> -->
                                ...
                                <tbody>
                                    @foreach($ac as $a)
                                    <tr data-id="{{ $a->id_tagihan_amb }}" 
                                        data-lokasi="{{ $a->lokasi }}" 
                                        data-kendaraan="{{ $a->id_kendaraan }}"
                                        data-merk="{{ $merkKendaraan[$a->id_kendaraan] ?? '-' }}"
                                        data-pemesan="{{ $a->pemesan }}"
                                        data-tgl_order="{{ $a->tgl_order }}"
                                        data-tgl_invoice="{{ $a->tgl_invoice }}"
                                        data-no_inventaris="{{ $a->no_inventaris }}"
                                        data-nama="{{ $a->nama }}"
                                        data-kategori="{{ $a->kategori }}"
                                        data-masa_pakai="{{ $a->masa_pakai }}"
                                        data-dipakai_untuk="{{ $a->dipakai_untuk }}"
                                        data-jml="{{ $a->jml }}"
                                        data-unit="{{ $a->id_satuan }}"
                                        data-harga="{{ 'Rp ' . number_format($a->harga ?? 0, 0, ',', '.') }}"
                                        data-total="{{ 'Rp ' . number_format($a->total ?? 0, 0, ',', '.') }}"
                                        data-toko="{{ $a->id_toko }}"
                                        data-file="{{ $a->file }}">
                                        <td><input type="checkbox" class="select-checkbox"></td>
                                        <td>{{ $loop->iteration }}.</td>
                                        <td>{{ $a->lokasi ?? '-' }}</td>
                                        <td>{{ $nopolKendaraan[$a->id_kendaraan] ?? '-' }}</td>
                                        <!-- <td>{{ $a->nopol ?? '-' }}</td>
                                        <td>{{ $a->kode_unit ?? '-' }}</td>
                                        <td>{{ $a->merk ?? '-' }}</td> -->
                                        @if ($a->file)
                                            <td>
                                                <a href="{{ asset('storage/' . $a->file) }}" target="_blank">{{ $a->pemesan ?? '-' }}</a>
                                            </td>
                                        @else
                                            <td>{{ $a->pemesan ?? '-' }}</td>
                                        @endif
                                        <td>{{ \Carbon\Carbon::createFromFormat('Y-m-d', $a->tgl_order)->format('d-M-Y') ?? '-' }}</td>
                                        <td>{{ \Carbon\Carbon::createFromFormat('Y-m-d', $a->tgl_invoice)->format('d-M-Y') ?? '-' }}</td>
                                        <td>{{ $a->no_inventaris ?? '-' }}</td>
                                        <td>{{ $a->nama ?? '-' }}</td>
                                        <td>{{ $a->kategori ?? '-' }}</td>
                                        <!-- <td>{{ $a->dipakai_untuk ?? '-' }}</td> -->
                                        <td>{{ $a->masa_pakai ?? '-' }}</td>
                                        <!-- <td>{{ $a->jml ?? '-' }}</td>
                                        <td>{{ $a->id_satuan ?? '-' }}</td> -->
                                        <td>{{ 'Rp ' . number_format($a->harga ?? 0, 0, ',', '.') }}</td>
                                        <td>{{ 'Rp ' . number_format($a->total ?? 0, 0, ',', '.') }}</td>
                                        <td>{{ $namaToko[$a->id_toko] ?? '-' }}</td>
                                        <td>
                                            @php
                                            ...
                                            @endphp
                                            <span class="{{ $statusClass }}">{{ ucfirst($a->status) }}</span>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
